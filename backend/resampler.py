...
class Resampler:
    ...
    def get_features(self):
        ...
        if flag_suffix:
            ...
            features_path = features_path.with_name(f"{self.in_file.stem}{cache_ext}")
        ...
        if features is not None:
            return features
        logging.info(
            f"{features_path} not found or forcing generation. Generating features."
        )
    ...
    def generate_features(self, features_path):
        ...
        if self._needs_hnsep_separation(breath, voicing, tension):
            logging.info(
                "Hb, Hv, or Ht flag requires hnsep separation. Split audio into breath, voicing"
            )

            hnsep_cache_path = self.in_file.with_name(f"{self.in_file.stem}_hnsep")

            force_generate = "G" in self.flags.keys()
            # Cache First

            seg_output = cache_manager.load_hnsep_cache(
                hnsep_cache_path, str(CONFIG.device), force_generate
            )
            ...
            if seg_output is None:
                ...
                seg_output = cache_manager.save_hnsep_cache(
                    hnsep_cache_path, seg_output
                )
            ...
            if tension != 0:
                ...
                wave = (breath / 100) * (wave - seg_output) + pre_emphasis_base_tension(
            ...
            # When breath == voicing and not equal to 100, apply simple scaling optimization.
            logging.info(
                f"Hb == Hv ({breath}), applying optimized simple scaling instead of hnsep separation"
            )
        ...
        if wave_max >= 0.5:
            ...
            # First reduce to a maximum of 0.5.
            scale = 0.5 / wave_max
            wave = wave * scale
            scale = scale.item()
        else:
        ...
        mel_origin = models.mel_analyzer(wave, gender / 100, 1).squeeze()
    ...
    def resample(self, features):
        ...
        if CONFIG.loop_mode or "He" in self.flags.keys():
            ...
            mel_loop = mel_origin[
                :,
                int((con + thop_origin / 2) // thop_origin) : int(
                    (end + thop_origin / 2) // thop_origin
                ),
            ]
            ...
            padded_mel = np.pad(
                mel_loop, pad_width=((0, 0), (0, int(pad_loop_size))), mode="reflect"
            )  # More pad
            logging.info(f"padded_mel: {padded_mel.shape}")
            mel_origin = np.concatenate(
                (
                    mel_origin[:, : int((con + thop_origin / 2) // thop_origin)],
                    padded_mel,
                ),
                axis=1,
            )
            ...
            t_area_origin = (
                np.arange(mel_origin.shape[1]) * thop_origin + thop_origin / 2
            )
            total_time = t_area_origin[-1] + thop_origin / 2
        ...
        stretched_n_frames = (
            con * vel + (total_time - con) * scaling_ratio
        ) // thop + 1
        ...
        # The number of frames in the MEL frame to the left of start
        ...
        # The number of MEL frames to the right of length_req+con
        end_right_mel_frames = (
            stretched_n_frames - (length_req + con * vel + thop / 2) // thop
        )
        ...
        stretched_t_mel = stretched_t_mel[
            int(cut_left_mel_frames) : int(stretched_n_frames - cut_right_mel_frames)
        ]
        logging.info(f"stretched_t_mel: {stretched_t_mel.shape}")
        ...
        if CONFIG.model_type == "ckpt":
            mel_render = (
                torch.from_numpy(mel_render).unsqueeze(0).to(dtype=torch.float32)
            )
            ...
            with torch.inference_mode():
                wav_con = models.vocoder.spec2wav_torch(
                    mel_render.to(CONFIG.device), f0=f0_render.to(CONFIG.device)
                )
            render = (
                wav_con[
                    int(new_start * CONFIG.sample_rate) : int(
                        new_end * CONFIG.sample_rate
                    )
                ]
                .to("cpu")
                .numpy()
            ...
            # Add a batch dimension to MEL and F0
            ...
            render = wav_con[
                int(new_start * CONFIG.sample_rate) : int(new_end * CONFIG.sample_rate)
            ]
            logging.info(f"cut_l:{int(new_start * CONFIG.sample_rate)}")
            logging.info(f"cut_r:{len(wav_con) - int(new_end * CONFIG.sample_rate)}")
            logging.info(
        ...
        if A_flag != 0:
            ...
            if len(pitch_render) > 1 and len(t) > 1:
                ...
                gain_at_mel_frames = 5 ** ((10**-4) * A_clamped * pitch_derivative)
