...
def load_sep_model(model_path, device=torch.device("cpu")):
    ...
    model = CascadedNet(
        ...
        is_complex=True,  # Multiple inputs
        is_mono=args_dict["is_mono"],
        fixed_length=True
        if args_dict.get("fixed_length", None) is None
...
def convert_to_onnx(model_path, output_path=None, device=torch.device("cpu")):
    ...
    # According to the requirement: Mono input with multiple values: [1, 1, 1024, 256]
    ...
    freq_bins = (
        args["n_fft"] // 2 + 1
    )  # 1025 for n_fft=2048, but we use 1024 in practice
    time_frames = 256

    dummy_input = torch.randn(
        batch_size, channels, freq_bins, time_frames, device=device
    )

    logging.info("Converting model to ONNX...")
    logging.info(f"Input shape: {dummy_input.shape}")
    logging.info(
        f"Model config: n_fft={args['n_fft']}, hop_length={args['hop_length']}"
    )
    logging.info(
        f"              n_out={args['n_out']}, n_out_lstm={args['n_out_lstm']}"
    )
